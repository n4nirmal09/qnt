	...
	var _globals = window.GreenSockGlobals || window,
			...
			for (i = 0; i < l; i++) {
				...
				if (!words) {
					...
					if (node._isSplit) {
						...
						nodes.splice(i--, 1);
						l--;
					} else if (!isChild && chars && !node._isSplit) {
						offset = (!lines && !absolute && node.nextSibling && node.nextSibling.nodeType !== 3); //if this is the last letter in the word (and we're not breaking by lines and not positioning things absolutely), we need to add a space afterwards so that the characters don't just mash together
		...
		_split = function(element, vars, allChars, allWords, allLines) {
			var children = _slice(element.childNodes),
				...
				wordIsOpen = true,
				text, splitText, i, j, character, child, hasTagStart, emojiPair1, emojiPair2;
			...
			if (element.nodeType !== 3 || l > 1) {
				...
				for (i = 0; i < l; i++) {
					...
					if (child.nodeType !== 3 || /\S+/.test(child.nodeValue)) {
						...
						child._isSplit = true;
						_split(child, vars, allChars, allWords); //don't split lines on child elements
					}
				}
				vars.absolute = absolute;
				if (!allLines) { //only do all the word/line parsing and absolute positioning on the top level. Remember, we don't pass in allLines array to the _split() of child nodes.
					return;
				}
			...
			container.innerHTML = element.nodeValue;
			...
			for (i = 0; i < l; i++) {
				...
				} else if (character.charCodeAt(0) >= _emojiStart && character.charCodeAt(0) <= _emojiEnd) { //special emoji characters use 2 or 4 unicode characters that we must keep together.
